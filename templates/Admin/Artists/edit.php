...
$this->assign('title', __('Edit') . ' ' . __('Artist'));
?>
				<div id="form-row" class="artists row">
					<div class="col-xs-12 col-xl-10">
						<div class="card mb-3">
							<div class="card-header">

								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-edit fa-spin"></i> <?= __('Edit') ?>: <?= __('Artist') ?></h3>
									<?= __('The data marked with <span class="fw-bold text-danger">*</span> must be provided!') ?>
								</div>

								<div class="float-end ms-5">
									<?= $this->Html->link('<span class="btn btn-outline-secondary mt-1 me-1"><i class="fa fa-times"></i></span>',
										["controller" => "Artists", "action" => "index", "_full" => true],
...
*/ ?>
							...
							<?= $this->Form->create($artist, ['id' => 'main-form']) ?>
								...
								<div class="card-body">
									...
									<div class="tab-content" id="tabContent">
										...
										<div class="tab-pane fade show active" id="tabPanelMain" role="tabpanel" aria-labelledby="tabMain" tabindex="0">
											...
											<!-- 2. STRING: artistJoin: string  required -->
											<div class="mb-3 form-group row text required">
												<label class="col-form-label col-md-2 pt-1 text-start text-md-end required" for="artistJoin"><?= __('ArtistJoin') ?>:</label>
												<div class="col-md-9">
													<?= $this->Form->control('artistJoin', ['label' => __('ArtistJoin'), 'placeholder' => __('ArtistJoin'), 'class' => 'form-control', 'empty' => false]); ?>

												</div>
											</div>

											<!-- 2. STRING: anv: string  required -->
											<div class="mb-3 form-group row text required">
												<label class="col-form-label col-md-2 pt-1 text-start text-md-end required" for="anv"><?= __('Anv') ?>:</label>
												<div class="col-md-9">
													<?= $this->Form->control('anv', ['label' => __('Anv'), 'placeholder' => __('Anv'), 'class' => 'form-control', 'empty' => false]); ?>

												</div>
											</div>

											<!-- 2. STRING: tracks: string  required -->
											<div class="mb-3 form-group row text required">
												<label class="col-form-label col-md-2 pt-1 text-start text-md-end required" for="tracks"><?= __('Tracks') ?>:</label>
												<div class="col-md-9">
													<?= $this->Form->control('tracks', ['label' => __('Tracks'), 'placeholder' => __('Tracks'), 'class' => 'form-control', 'empty' => false]); ?>
											...
											<!-- 3. INTEGER: resourceId: integer  required -->
											<div class="mb-3 form-group row number required">
												<label class="col-form-label col-md-2 pt-1 text-start text-md-end required" for="resourceId"><?= __('ResourceId') ?>:</label>
												<div class="col-xs-12 col-sm-12 col-md-5 col-lg-4 col-xl-4 col-xxl-4">
													<?= $this->Form->control('resourceId', ['class' => 'form-control', 'placeholder' => __('ResourceId'), 'data-decimals' => '0', 'min' => '0', 'max' => '999999999999', 'step' => '1', 'empty' => false]); ?>
...
<?php $this->Html->scriptStart(['block' => 'javaScriptBottom']); ?>
	...
	jeffAdminInitDateTimePicker('last_used'<?= $artist->last_used !== null ? ", '" . $artist->last_used->format('Y-m-d H:i:s') . "'" : "" ?>)
