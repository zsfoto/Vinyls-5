						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Albums Language') ?></h3>
...
<?php } ?>
											<div class="row"><!-- 1. -->
												...
												<div class="col-sm-10 p-1 link">
													<?= $albumsLanguage->hasValue('album') ? $this->Html->link($albumsLanguage->album->name, ['controller' => 'Albums', 'action' => 'view', $albumsLanguage->album->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Language') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $albumsLanguage->hasValue('language') ? $this->Html->link($albumsLanguage->language->name, ['controller' => 'Languages', 'action' => 'view', $albumsLanguage->language->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 4. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Last Used') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= h($albumsLanguage->last_used) ?>
