						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Albums Users Abbreviation') ?></h3>
...
<?php if($config['show_id']){ ?>
											<div class="row"><!-- 3. -->
												...
												<div class="col-sm-10 p-1">
													<?= $albumsUsersAbbreviation->id ?><!-- 0.a -->
												</div>
											</div>
<?php } ?>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Albums User') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $albumsUsersAbbreviation->hasValue('albums_user') ? $this->Html->link($albumsUsersAbbreviation->albums_user->id, ['controller' => 'AlbumsUsers', 'action' => 'view', $albumsUsersAbbreviation->albums_user->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Abbreviation') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $albumsUsersAbbreviation->hasValue('abbreviation') ? $this->Html->link($albumsUsersAbbreviation->abbreviation->name, ['controller' => 'Abbreviations', 'action' => 'view', $albumsUsersAbbreviation->abbreviation->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 4. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Last Used') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= h($albumsUsersAbbreviation->last_used) ?>
