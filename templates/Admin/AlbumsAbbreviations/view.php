 ...
 * @var \App\Model\Entity\AlbumsAbbreviation $albumsAbbreviation
 */
						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Albums Abbreviation') ?></h3>
									<?= __('The data marked with <span class="fw-bold text-danger">*</span> must be provided!') ?>
								</div>
								<div class="float-end ms-5">
									<?= $this->Html->link('<span class="btn btn-outline-secondary mt-1 me-1"><i class="fa fa-times"></i></span>',
										["controller" => "Albums Abbreviations", "action" => "index", "_full" => true],
...
<?php } ?>
											<div class="row"><!-- 1. -->
												...
												<div class="col-sm-10 p-1 link">
													<?= $albumsAbbreviation->hasValue('album') ? $this->Html->link($albumsAbbreviation->album->name, ['controller' => 'Albums', 'action' => 'view', $albumsAbbreviation->album->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Abbreviation') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $albumsAbbreviation->hasValue('abbreviation') ? $this->Html->link($albumsAbbreviation->abbreviation->name, ['controller' => 'Abbreviations', 'action' => 'view', $albumsAbbreviation->abbreviation->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 4. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Last Used') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= h($albumsAbbreviation->last_used) ?>
