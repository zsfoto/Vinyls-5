						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Albums Style') ?></h3>
...
<?php } ?>
											<div class="row"><!-- 1. -->
												...
												<div class="col-sm-10 p-1 link">
													<?= $albumsStyle->hasValue('album') ? $this->Html->link($albumsStyle->album->name, ['controller' => 'Albums', 'action' => 'view', $albumsStyle->album->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Style') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $albumsStyle->hasValue('style') ? $this->Html->link($albumsStyle->style->name, ['controller' => 'Styles', 'action' => 'view', $albumsStyle->style->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 4. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Last Used') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= h($albumsStyle->last_used) ?>
