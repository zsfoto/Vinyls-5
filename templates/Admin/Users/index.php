...
<?php } ?>
											<th class="string usergroup-id"><?= $this->Paginator->sort('usergroup_id') ?></th><!-- H.0. -->
											...
											<th class="datetime token-expire"><?= $this->Paginator->sort('token_expire') ?></th><!-- H.1. -->
...
<?php } ?>
											<td class="string link usergroup-id" value="<?= $user->usergroup_id ?>"><?= $user->hasValue('usergroup') ? $this->Html->link($user->usergroup->name, ['controller' => 'Usergroups', 'action' => 'view', $user->usergroup->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span></td>
											<td class="string name" value="<?= $user->name ?>"><?= h($user->name) ?></td>
											<td class="string email" value="<?= $user->email ?>"><?= h($user->email) ?></td>
											<td class="boolean enabled" value="<?= $user->enabled ?>"><?= h($user->enabled) ?></td>
											<td class="datetime token-expire" value="<?= $user->token_expire ?>"><?= h($user->token_expire) ?></td>
<?php if($config['show_pos']){ ?>
...
<?php } ?>
<?php if($config['show_counters']){ ?>
											<td class="number counter album-count" value="<?= $user->album_count ?>"><?= h($user->album_count) ?></td><?php } ?>
<?php if($config['show_created'] || $config['show_modified']){ ?>
											<td class="datetime">
