						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Extraartists Tracklist') ?></h3>
...
<?php if($config['show_id']){ ?>
											<div class="row"><!-- 3. -->
												...
												<div class="col-sm-10 p-1">
													<?= $extraartistsTracklist->id ?><!-- 0.a -->
												</div>
											</div>
<?php } ?>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Tracklist') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $extraartistsTracklist->hasValue('tracklist') ? $this->Html->link($extraartistsTracklist->tracklist->title, ['controller' => 'Tracklists', 'action' => 'view', $extraartistsTracklist->tracklist->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 2. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Role') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= h($extraartistsTracklist->role) ?>

												</div>
											</div>
											<div class="row"><!-- 3. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Extraartist Id') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= $this->Number->format($extraartistsTracklist->extraartist_id) ?><!-- 3.b -->
												</div>
											</div>
											<div class="row"><!-- 4. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Last Used') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= h($extraartistsTracklist->last_used) ?>
