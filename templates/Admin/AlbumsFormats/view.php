						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Albums Format') ?></h3>
...
<?php } ?>
											<div class="row"><!-- 1. -->
												...
												<div class="col-sm-10 p-1 link">
													<?= $albumsFormat->hasValue('album') ? $this->Html->link($albumsFormat->album->name, ['controller' => 'Albums', 'action' => 'view', $albumsFormat->album->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Format') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $albumsFormat->hasValue('format') ? $this->Html->link($albumsFormat->format->name, ['controller' => 'Formats', 'action' => 'view', $albumsFormat->format->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 4. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Last Used') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= h($albumsFormat->last_used) ?>
