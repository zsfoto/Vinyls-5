...
/**
 * Tracklists Controller
 *
 * @property \App\Model\Table\TracklistsTable $Tracklists
 */
class TracklistsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Tracklists'));
		...
		$this->paginate['Tracklists']['page'] 	= $page;
		$this->paginate['Tracklists']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			$showSearchBar	 = true;
			$query = $this->Tracklists->find()
				->contain(['Albums', 'Conditions'])
				->where([
					'OR' => [
						'name LIKE' => '%' . $search .  '%',
						//'title LIKE' => '%' . $search .  '%',
			...
			$query = $this->Tracklists->find()->contain(['Albums', 'Conditions'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Tracklists']['limit'] = $this->config['paginate_limit'];
			$tracklists = $this->paginate($query);
		...
		if(empty($tracklists->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('tracklists'));
    }

    /**
     * View method
     *
     * @param string|null $id Tracklist id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('tracklist') . ' ' . __('record'));
        $tracklist = $this->Tracklists->get($id, contain: ['Albums', 'Conditions', 'ExtraartistsTracklists', 'Lyrics']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $tracklist->name;
        $this->set(compact('tracklist', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('tracklist') . ' ' . __('record'));
        $tracklist = $this->Tracklists->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $tracklist = $this->Tracklists->patchEntity($tracklist, $data);
			//debug($tracklist);
			//die();
            if ($this->Tracklists->save($tracklist)) {
                //$this->Flash->success(__('The tracklist has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $tracklist->id);
            ...
            //$this->Flash->error(__('The tracklist could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $albums = $this->Tracklists->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $conditions = $this->Tracklists->Conditions->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('tracklist', 'albums', 'conditions'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('tracklist') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $tracklist = $this->Tracklists->get($id, contain: []);
        ...
        $conditions = $this->Tracklists->Conditions->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $tracklist->name;
        $this->set(compact('tracklist', 'albums', 'conditions', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $tracklist = $this->Tracklists->get($id);
        if ($this->Tracklists->delete($tracklist)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
            //$this->Flash->success(__('The tracklist has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The tracklist could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
