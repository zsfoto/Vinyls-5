...
class ShelvesController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Shelves'));
		...
		$this->paginate['Shelves']['page'] 	= $page;
		$this->paginate['Shelves']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			...
			$query = $this->Shelves->find()->contain(['Users'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Shelves']['limit'] = $this->config['paginate_limit'];
			$shelves = $this->paginate($query);
		...
		if(empty($shelves->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('shelves'));
    }
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('shelf') . ' ' . __('record'));
        $shelf = $this->Shelves->get($id, contain: ['Users', 'AlbumsUsers']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $shelf->name;
        $this->set(compact('shelf', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('shelf') . ' ' . __('record'));
        $shelf = $this->Shelves->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $shelf = $this->Shelves->patchEntity($shelf, $data);
			//debug($shelf);
			//die();
            if ($this->Shelves->save($shelf)) {
                //$this->Flash->success(__('The shelf has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $shelf->id);
            ...
            //$this->Flash->error(__('The shelf could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $users = $this->Shelves->Users->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('shelf') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $shelf = $this->Shelves->get($id, contain: []);
        ...
        $users = $this->Shelves->Users->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $shelf->name;
        $this->set(compact('shelf', 'users', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Shelves->delete($shelf)) {
            ...
            //$this->Flash->success(__('The shelf has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The shelf could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
