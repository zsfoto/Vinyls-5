...
class ConditionsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Conditions'));
		...
		$this->paginate['Conditions']['page'] 	= $page;
		$this->paginate['Conditions']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			...
			$query = $this->Conditions->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Conditions']['limit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('condition') . ' ' . __('record'));
        $condition = $this->Conditions->get($id, contain: ['AlbumsUsers', 'Tracklists']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $condition->name;
        $this->set(compact('condition', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('condition') . ' ' . __('record'));
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Conditions->save($condition)) {
                //$this->Flash->success(__('The condition has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $condition->id);
            ...
            //$this->Flash->error(__('The condition could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('condition') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Conditions->delete($condition)) {
            ...
            //$this->Flash->success(__('The condition has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The condition could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
