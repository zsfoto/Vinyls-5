...
class UsergroupsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Usergroups'));
		...
		$this->paginate['Usergroups']['page'] 	= $page;
		$this->paginate['Usergroups']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			...
			$query = $this->Usergroups->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Usergroups']['limit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('usergroup') . ' ' . __('record'));
        $usergroup = $this->Usergroups->get($id, contain: ['Users']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $usergroup->name;
        $this->set(compact('usergroup', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('usergroup') . ' ' . __('record'));
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Usergroups->save($usergroup)) {
                //$this->Flash->success(__('The usergroup has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $usergroup->id);
            ...
            //$this->Flash->error(__('The usergroup could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('usergroup') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Usergroups->delete($usergroup)) {
            ...
            //$this->Flash->success(__('The usergroup has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The usergroup could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
