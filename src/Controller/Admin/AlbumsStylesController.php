...
/**
 * AlbumsStyles Controller
 *
 * @property \App\Model\Table\AlbumsStylesTable $AlbumsStyles
 */
class AlbumsStylesController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('AlbumsStyles'));
		...
		$this->paginate['AlbumsStyles']['page'] 	= $page;
		$this->paginate['AlbumsStyles']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			$showSearchBar	 = true;
			$query = $this->AlbumsStyles->find()
				->contain(['Albums', 'Styles'])
				->where([
					'OR' => [
						'name LIKE' => '%' . $search .  '%',
						//'title LIKE' => '%' . $search .  '%',
			...
			$query = $this->AlbumsStyles->find()->contain(['Albums', 'Styles'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['AlbumsStyles']['limit'] = $this->config['paginate_limit'];
			$albumsStyles = $this->paginate($query);
		...
		if(empty($albumsStyles->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('albumsStyles'));
    }

    /**
     * View method
     *
     * @param string|null $id Albums Style id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('albumsStyle') . ' ' . __('record'));
        $albumsStyle = $this->AlbumsStyles->get($id, contain: ['Albums', 'Styles']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $albumsStyle->name;
        $this->set(compact('albumsStyle', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('albumsStyle') . ' ' . __('record'));
        $albumsStyle = $this->AlbumsStyles->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $albumsStyle = $this->AlbumsStyles->patchEntity($albumsStyle, $data);
			//debug($albumsStyle);
			//die();
            if ($this->AlbumsStyles->save($albumsStyle)) {
                //$this->Flash->success(__('The albums style has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $albumsStyle->id);
            ...
            //$this->Flash->error(__('The albums style could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $albums = $this->AlbumsStyles->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $styles = $this->AlbumsStyles->Styles->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('albumsStyle', 'albums', 'styles'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('albumsStyle') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $albumsStyle = $this->AlbumsStyles->get($id, contain: []);
        ...
        $styles = $this->AlbumsStyles->Styles->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $albumsStyle->name;
        $this->set(compact('albumsStyle', 'albums', 'styles', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $albumsStyle = $this->AlbumsStyles->get($id);
        if ($this->AlbumsStyles->delete($albumsStyle)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
            //$this->Flash->success(__('The albums style has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The albums style could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
