...
class StylesController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Styles'));
		...
		$this->paginate['Styles']['page'] 	= $page;
		$this->paginate['Styles']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			...
			$query = $this->Styles->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Styles']['limit'] = $this->config['paginate_limit'];
			$styles = $this->paginate($query);
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('style') . ' ' . __('record'));
        $style = $this->Styles->get($id, contain: ['Albums']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $style->name;
        $this->set(compact('style', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('style') . ' ' . __('record'));
        $style = $this->Styles->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $style = $this->Styles->patchEntity($style, $data);
			//debug($style);
			//die();
            if ($this->Styles->save($style)) {
                //$this->Flash->success(__('The style has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $style->id);
            ...
            //$this->Flash->error(__('The style could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $albums = $this->Styles->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('style', 'albums'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('style') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
        ...
        $albums = $this->Styles->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $style->name;
        $this->set(compact('style', 'albums', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Styles->delete($style)) {
            ...
            //$this->Flash->success(__('The style has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The style could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
