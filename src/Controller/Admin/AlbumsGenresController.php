...
/**
 * AlbumsGenres Controller
 *
 * @property \App\Model\Table\AlbumsGenresTable $AlbumsGenres
 */
class AlbumsGenresController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('AlbumsGenres'));
		...
		$this->paginate['AlbumsGenres']['page'] 	= $page;
		$this->paginate['AlbumsGenres']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			$showSearchBar	 = true;
			$query = $this->AlbumsGenres->find()
				->contain(['Albums', 'Genres'])
				->where([
					'OR' => [
						'name LIKE' => '%' . $search .  '%',
						//'title LIKE' => '%' . $search .  '%',
			...
			$query = $this->AlbumsGenres->find()->contain(['Albums', 'Genres'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['AlbumsGenres']['limit'] = $this->config['paginate_limit'];
			$albumsGenres = $this->paginate($query);
		...
		if(empty($albumsGenres->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('albumsGenres'));
    }

    /**
     * View method
     *
     * @param string|null $id Albums Genre id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('albumsGenre') . ' ' . __('record'));
        $albumsGenre = $this->AlbumsGenres->get($id, contain: ['Albums', 'Genres']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $albumsGenre->name;
        $this->set(compact('albumsGenre', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('albumsGenre') . ' ' . __('record'));
        $albumsGenre = $this->AlbumsGenres->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $albumsGenre = $this->AlbumsGenres->patchEntity($albumsGenre, $data);
			//debug($albumsGenre);
			//die();
            if ($this->AlbumsGenres->save($albumsGenre)) {
                //$this->Flash->success(__('The albums genre has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $albumsGenre->id);
            ...
            //$this->Flash->error(__('The albums genre could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $albums = $this->AlbumsGenres->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $genres = $this->AlbumsGenres->Genres->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('albumsGenre', 'albums', 'genres'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('albumsGenre') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $albumsGenre = $this->AlbumsGenres->get($id, contain: []);
        ...
        $genres = $this->AlbumsGenres->Genres->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $albumsGenre->name;
        $this->set(compact('albumsGenre', 'albums', 'genres', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $albumsGenre = $this->AlbumsGenres->get($id);
        if ($this->AlbumsGenres->delete($albumsGenre)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
            //$this->Flash->success(__('The albums genre has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The albums genre could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
