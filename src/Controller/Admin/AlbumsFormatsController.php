...
/**
 * AlbumsFormats Controller
 *
 * @property \App\Model\Table\AlbumsFormatsTable $AlbumsFormats
 */
class AlbumsFormatsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('AlbumsFormats'));
		...
		$this->paginate['AlbumsFormats']['page'] 	= $page;
		$this->paginate['AlbumsFormats']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			$showSearchBar	 = true;
			$query = $this->AlbumsFormats->find()
				->contain(['Albums', 'Formats'])
				->where([
					'OR' => [
						'name LIKE' => '%' . $search .  '%',
						//'title LIKE' => '%' . $search .  '%',
			...
			$query = $this->AlbumsFormats->find()->contain(['Albums', 'Formats'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['AlbumsFormats']['limit'] = $this->config['paginate_limit'];
			$albumsFormats = $this->paginate($query);
		...
		if(empty($albumsFormats->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('albumsFormats'));
    }

    /**
     * View method
     *
     * @param string|null $id Albums Format id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('albumsFormat') . ' ' . __('record'));
        $albumsFormat = $this->AlbumsFormats->get($id, contain: ['Albums', 'Formats']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $albumsFormat->name;
        $this->set(compact('albumsFormat', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('albumsFormat') . ' ' . __('record'));
        $albumsFormat = $this->AlbumsFormats->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $albumsFormat = $this->AlbumsFormats->patchEntity($albumsFormat, $data);
			//debug($albumsFormat);
			//die();
            if ($this->AlbumsFormats->save($albumsFormat)) {
                //$this->Flash->success(__('The albums format has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $albumsFormat->id);
            ...
            //$this->Flash->error(__('The albums format could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $albums = $this->AlbumsFormats->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $formats = $this->AlbumsFormats->Formats->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('albumsFormat', 'albums', 'formats'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('albumsFormat') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $albumsFormat = $this->AlbumsFormats->get($id, contain: []);
        ...
        $formats = $this->AlbumsFormats->Formats->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $albumsFormat->name;
        $this->set(compact('albumsFormat', 'albums', 'formats', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $albumsFormat = $this->AlbumsFormats->get($id);
        if ($this->AlbumsFormats->delete($albumsFormat)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
            //$this->Flash->success(__('The albums format has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The albums format could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
