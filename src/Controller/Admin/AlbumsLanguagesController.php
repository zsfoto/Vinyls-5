...
/**
 * AlbumsLanguages Controller
 *
 * @property \App\Model\Table\AlbumsLanguagesTable $AlbumsLanguages
 */
class AlbumsLanguagesController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('AlbumsLanguages'));
		...
		$this->paginate['AlbumsLanguages']['page'] 	= $page;
		$this->paginate['AlbumsLanguages']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			$showSearchBar	 = true;
			$query = $this->AlbumsLanguages->find()
				->contain(['Albums', 'Languages'])
				->where([
					'OR' => [
						'name LIKE' => '%' . $search .  '%',
						//'title LIKE' => '%' . $search .  '%',
			...
			$query = $this->AlbumsLanguages->find()->contain(['Albums', 'Languages'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['AlbumsLanguages']['limit'] = $this->config['paginate_limit'];
			$albumsLanguages = $this->paginate($query);
		...
		if(empty($albumsLanguages->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('albumsLanguages'));
    }

    /**
     * View method
     *
     * @param string|null $id Albums Language id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('albumsLanguage') . ' ' . __('record'));
        $albumsLanguage = $this->AlbumsLanguages->get($id, contain: ['Albums', 'Languages']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $albumsLanguage->name;
        $this->set(compact('albumsLanguage', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('albumsLanguage') . ' ' . __('record'));
        $albumsLanguage = $this->AlbumsLanguages->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $albumsLanguage = $this->AlbumsLanguages->patchEntity($albumsLanguage, $data);
			//debug($albumsLanguage);
			//die();
            if ($this->AlbumsLanguages->save($albumsLanguage)) {
                //$this->Flash->success(__('The albums language has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $albumsLanguage->id);
            ...
            //$this->Flash->error(__('The albums language could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $albums = $this->AlbumsLanguages->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $languages = $this->AlbumsLanguages->Languages->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('albumsLanguage', 'albums', 'languages'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('albumsLanguage') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $albumsLanguage = $this->AlbumsLanguages->get($id, contain: []);
        ...
        $languages = $this->AlbumsLanguages->Languages->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $albumsLanguage->name;
        $this->set(compact('albumsLanguage', 'albums', 'languages', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $albumsLanguage = $this->AlbumsLanguages->get($id);
        if ($this->AlbumsLanguages->delete($albumsLanguage)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
            //$this->Flash->success(__('The albums language has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The albums language could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
