...
class LabelsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Labels'));
		...
		$this->paginate['Labels']['page'] 	= $page;
		$this->paginate['Labels']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			...
			$query = $this->Labels->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Labels']['limit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('label') . ' ' . __('record'));
        $label = $this->Labels->get($id, contain: ['Albums']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $label->name;
        $this->set(compact('label', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('label') . ' ' . __('record'));
        $label = $this->Labels->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $label = $this->Labels->patchEntity($label, $data);
			//debug($label);
			//die();
            if ($this->Labels->save($label)) {
                //$this->Flash->success(__('The label has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $label->id);
            ...
            //$this->Flash->error(__('The label could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $albums = $this->Labels->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('label', 'albums'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('label') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
        ...
        $albums = $this->Labels->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $label->name;
        $this->set(compact('label', 'albums', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Labels->delete($label)) {
            ...
            //$this->Flash->success(__('The label has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The label could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
