...
class AlbumsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Albums'));
		...
		$this->paginate['Albums']['page'] 	= $page;
		$this->paginate['Albums']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			...
			$query = $this->Albums->find()->contain(['Countries'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Albums']['limit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('album') . ' ' . __('record'));
        $album = $this->Albums->get($id, contain: ['Countries', 'Abbreviations', 'Artists', 'Companies', 'Formats', 'Genres', 'Labels', 'Languages', 'Styles', 'Users', 'AlbumsExtraartists', 'Identifiers', 'Images', 'Tracklists', 'Videos']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $album->name;
        $this->set(compact('album', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('album') . ' ' . __('record'));
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Albums->save($album)) {
                //$this->Flash->success(__('The album has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $album->id);
            ...
            //$this->Flash->error(__('The album could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $countries = $this->Albums->Countries->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $abbreviations = $this->Albums->Abbreviations->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $artists = $this->Albums->Artists->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $companies = $this->Albums->Companies->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $formats = $this->Albums->Formats->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $genres = $this->Albums->Genres->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $labels = $this->Albums->Labels->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $languages = $this->Albums->Languages->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $styles = $this->Albums->Styles->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $users = $this->Albums->Users->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('album', 'countries', 'abbreviations', 'artists', 'companies', 'formats', 'genres', 'labels', 'languages', 'styles', 'users'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('album') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $album = $this->Albums->get($id, contain: ['Abbreviations', 'Artists', 'Companies', 'Formats', 'Genres', 'Labels', 'Languages', 'Styles', 'Users']);
        ...
        $users = $this->Albums->Users->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $album->name;
        $this->set(compact('album', 'countries', 'abbreviations', 'artists', 'companies', 'formats', 'genres', 'labels', 'languages', 'styles', 'users', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Albums->delete($album)) {
            ...
            //$this->Flash->success(__('The album has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The album could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
