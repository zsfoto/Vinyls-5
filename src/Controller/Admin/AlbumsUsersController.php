...
/**
 * AlbumsUsers Controller
 *
 * @property \App\Model\Table\AlbumsUsersTable $AlbumsUsers
 */
class AlbumsUsersController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('AlbumsUsers'));
		...
		$this->paginate['AlbumsUsers']['page'] 	= $page;
		$this->paginate['AlbumsUsers']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			$showSearchBar	 = true;
			$query = $this->AlbumsUsers->find()
				->contain(['Users', 'Albums', 'Conditions', 'Shelves'])
				->where([
					'OR' => [
						'name LIKE' => '%' . $search .  '%',
						//'title LIKE' => '%' . $search .  '%',
			...
			$query = $this->AlbumsUsers->find()->contain(['Users', 'Albums', 'Conditions', 'Shelves'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['AlbumsUsers']['limit'] = $this->config['paginate_limit'];
			$albumsUsers = $this->paginate($query);
		...
		if(empty($albumsUsers->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('albumsUsers'));
    }

    /**
     * View method
     *
     * @param string|null $id Albums User id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('albumsUser') . ' ' . __('record'));
        $albumsUser = $this->AlbumsUsers->get($id, contain: ['Users', 'Albums', 'Conditions', 'Shelves', 'Abbreviations']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $albumsUser->name;
        $this->set(compact('albumsUser', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('albumsUser') . ' ' . __('record'));
        $albumsUser = $this->AlbumsUsers->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $albumsUser = $this->AlbumsUsers->patchEntity($albumsUser, $data);
			//debug($albumsUser);
			//die();
            if ($this->AlbumsUsers->save($albumsUser)) {
                //$this->Flash->success(__('The albums user has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $albumsUser->id);
            ...
            //$this->Flash->error(__('The albums user could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $users = $this->AlbumsUsers->Users->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $albums = $this->AlbumsUsers->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $conditions = $this->AlbumsUsers->Conditions->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $shelves = $this->AlbumsUsers->Shelves->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $abbreviations = $this->AlbumsUsers->Abbreviations->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('albumsUser', 'users', 'albums', 'conditions', 'shelves', 'abbreviations'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('albumsUser') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $albumsUser = $this->AlbumsUsers->get($id, contain: ['Abbreviations']);
        ...
        $abbreviations = $this->AlbumsUsers->Abbreviations->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $albumsUser->name;
        $this->set(compact('albumsUser', 'users', 'albums', 'conditions', 'shelves', 'abbreviations', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $albumsUser = $this->AlbumsUsers->get($id);
        if ($this->AlbumsUsers->delete($albumsUser)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
            //$this->Flash->success(__('The albums user has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The albums user could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
