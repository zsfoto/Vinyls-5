...
/**
 * ExtraartistsTracklists Controller
 *
 * @property \App\Model\Table\ExtraartistsTracklistsTable $ExtraartistsTracklists
 */
class ExtraartistsTracklistsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('ExtraartistsTracklists'));
		...
		$this->paginate['ExtraartistsTracklists']['page'] 	= $page;
		$this->paginate['ExtraartistsTracklists']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			$showSearchBar	 = true;
			$query = $this->ExtraartistsTracklists->find()
				->contain(['Tracklists'])
				->where([
					'OR' => [
						'name LIKE' => '%' . $search .  '%',
			...
			$query = $this->ExtraartistsTracklists->find()->contain(['Tracklists'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['ExtraartistsTracklists']['limit'] = $this->config['paginate_limit'];
			$extraartistsTracklists = $this->paginate($query);
		...
		if(empty($extraartistsTracklists->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('extraartistsTracklists'));
    }

    /**
     * View method
     *
     * @param string|null $id Extraartists Tracklist id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('extraartistsTracklist') . ' ' . __('record'));
        $extraartistsTracklist = $this->ExtraartistsTracklists->get($id, contain: ['Tracklists']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $extraartistsTracklist->name;
        $this->set(compact('extraartistsTracklist', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('extraartistsTracklist') . ' ' . __('record'));
        $extraartistsTracklist = $this->ExtraartistsTracklists->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $extraartistsTracklist = $this->ExtraartistsTracklists->patchEntity($extraartistsTracklist, $data);
			//debug($extraartistsTracklist);
			//die();
            if ($this->ExtraartistsTracklists->save($extraartistsTracklist)) {
                //$this->Flash->success(__('The extraartists tracklist has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $extraartistsTracklist->id);

                //return $this->redirect(['action' => 'add']);
                return $this->redirect([
					'controller' => $this->controller,
					'action' => 'index',
					'#' => $extraartistsTracklist->id
				]);

            }
            //$this->Flash->error(__('The extraartists tracklist could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $tracklists = $this->ExtraartistsTracklists->Tracklists->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('extraartistsTracklist', 'tracklists'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('extraartistsTracklist') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $extraartistsTracklist = $this->ExtraartistsTracklists->get($id, contain: []);
        ...
        $tracklists = $this->ExtraartistsTracklists->Tracklists->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $extraartistsTracklist->name;
        $this->set(compact('extraartistsTracklist', 'tracklists', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $extraartistsTracklist = $this->ExtraartistsTracklists->get($id);
        if ($this->ExtraartistsTracklists->delete($extraartistsTracklist)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
            //$this->Flash->success(__('The extraartists tracklist has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The extraartists tracklist could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
