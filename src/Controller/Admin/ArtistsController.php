...
class ArtistsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Artists'));
		...
		$this->paginate['Artists']['page'] 	= $page;
		$this->paginate['Artists']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			...
			$query = $this->Artists->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Artists']['limit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('artist') . ' ' . __('record'));
        $artist = $this->Artists->get($id, contain: ['Albums']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $artist->name;
        $this->set(compact('artist', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('artist') . ' ' . __('record'));
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Artists->save($artist)) {
                //$this->Flash->success(__('The artist has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $artist->id);
            ...
            //$this->Flash->error(__('The artist could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $albums = $this->Artists->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('artist') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
        ...
        $albums = $this->Artists->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $artist->name;
        $this->set(compact('artist', 'albums', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Artists->delete($artist)) {
            ...
            //$this->Flash->success(__('The artist has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The artist could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
