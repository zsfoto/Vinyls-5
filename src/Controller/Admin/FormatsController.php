...
class FormatsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Formats'));
		...
		$this->paginate['Formats']['page'] 	= $page;
		$this->paginate['Formats']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			...
			$query = $this->Formats->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Formats']['limit'] = $this->config['paginate_limit'];
			$formats = $this->paginate($query);
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('format') . ' ' . __('record'));
        $format = $this->Formats->get($id, contain: ['Albums']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $format->name;
        $this->set(compact('format', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('format') . ' ' . __('record'));
        $format = $this->Formats->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $format = $this->Formats->patchEntity($format, $data);
			//debug($format);
			//die();
            if ($this->Formats->save($format)) {
                //$this->Flash->success(__('The format has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $format->id);
            ...
            //$this->Flash->error(__('The format could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $albums = $this->Formats->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('format', 'albums'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('format') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
        ...
        $albums = $this->Formats->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $format->name;
        $this->set(compact('format', 'albums', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Formats->delete($format)) {
            ...
            //$this->Flash->success(__('The format has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The format could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
