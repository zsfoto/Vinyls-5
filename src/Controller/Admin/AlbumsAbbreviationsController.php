...
/**
 * AlbumsAbbreviations Controller
 *
 * @property \App\Model\Table\AlbumsAbbreviationsTable $AlbumsAbbreviations
 */
class AlbumsAbbreviationsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('AlbumsAbbreviations'));
		...
		$this->paginate['AlbumsAbbreviations']['page'] 	= $page;
		$this->paginate['AlbumsAbbreviations']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			$showSearchBar	 = true;
			$query = $this->AlbumsAbbreviations->find()
				->contain(['Albums', 'Abbreviations'])
				->where([
					'OR' => [
						'name LIKE' => '%' . $search .  '%',
						//'title LIKE' => '%' . $search .  '%',
			...
			$query = $this->AlbumsAbbreviations->find()->contain(['Albums', 'Abbreviations'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['AlbumsAbbreviations']['limit'] = $this->config['paginate_limit'];
			$albumsAbbreviations = $this->paginate($query);
		...
		if(empty($albumsAbbreviations->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('albumsAbbreviations'));
    }

    /**
     * View method
     *
     * @param string|null $id Albums Abbreviation id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('albumsAbbreviation') . ' ' . __('record'));
        $albumsAbbreviation = $this->AlbumsAbbreviations->get($id, contain: ['Albums', 'Abbreviations']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $albumsAbbreviation->name;
        $this->set(compact('albumsAbbreviation', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('albumsAbbreviation') . ' ' . __('record'));
        $albumsAbbreviation = $this->AlbumsAbbreviations->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $albumsAbbreviation = $this->AlbumsAbbreviations->patchEntity($albumsAbbreviation, $data);
			//debug($albumsAbbreviation);
			//die();
            if ($this->AlbumsAbbreviations->save($albumsAbbreviation)) {
                //$this->Flash->success(__('The albums abbreviation has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $albumsAbbreviation->id);

                //return $this->redirect(['action' => 'add']);
                return $this->redirect([
					'controller' => $this->controller,
					'action' => 'index',
					'#' => $albumsAbbreviation->id
				]);

            }
            //$this->Flash->error(__('The albums abbreviation could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $albums = $this->AlbumsAbbreviations->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $abbreviations = $this->AlbumsAbbreviations->Abbreviations->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('albumsAbbreviation', 'albums', 'abbreviations'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('albumsAbbreviation') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $albumsAbbreviation = $this->AlbumsAbbreviations->get($id, contain: []);
        ...
        $abbreviations = $this->AlbumsAbbreviations->Abbreviations->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $albumsAbbreviation->name;
        $this->set(compact('albumsAbbreviation', 'albums', 'abbreviations', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $albumsAbbreviation = $this->AlbumsAbbreviations->get($id);
        if ($this->AlbumsAbbreviations->delete($albumsAbbreviation)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
            //$this->Flash->success(__('The albums abbreviation has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The albums abbreviation could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
