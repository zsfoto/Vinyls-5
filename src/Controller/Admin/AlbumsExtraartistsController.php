...
/**
 * AlbumsExtraartists Controller
 *
 * @property \App\Model\Table\AlbumsExtraartistsTable $AlbumsExtraartists
 */
class AlbumsExtraartistsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('AlbumsExtraartists'));
		...
		$this->paginate['AlbumsExtraartists']['page'] 	= $page;
		$this->paginate['AlbumsExtraartists']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			$showSearchBar	 = true;
			$query = $this->AlbumsExtraartists->find()
				->contain(['Albums'])
				->where([
					'OR' => [
						'name LIKE' => '%' . $search .  '%',
			...
			$query = $this->AlbumsExtraartists->find()->contain(['Albums'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['AlbumsExtraartists']['limit'] = $this->config['paginate_limit'];
			$albumsExtraartists = $this->paginate($query);
		...
		if(empty($albumsExtraartists->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('albumsExtraartists'));
    }

    /**
     * View method
     *
     * @param string|null $id Albums Extraartist id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('albumsExtraartist') . ' ' . __('record'));
        $albumsExtraartist = $this->AlbumsExtraartists->get($id, contain: ['Albums']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $albumsExtraartist->name;
        $this->set(compact('albumsExtraartist', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('albumsExtraartist') . ' ' . __('record'));
        $albumsExtraartist = $this->AlbumsExtraartists->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $albumsExtraartist = $this->AlbumsExtraartists->patchEntity($albumsExtraartist, $data);
			//debug($albumsExtraartist);
			//die();
            if ($this->AlbumsExtraartists->save($albumsExtraartist)) {
                //$this->Flash->success(__('The albums extraartist has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $albumsExtraartist->id);

                //return $this->redirect(['action' => 'add']);
                return $this->redirect([
					'controller' => $this->controller,
					'action' => 'index',
					'#' => $albumsExtraartist->id
				]);

            }
            //$this->Flash->error(__('The albums extraartist could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $albums = $this->AlbumsExtraartists->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('albumsExtraartist', 'albums'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('albumsExtraartist') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $albumsExtraartist = $this->AlbumsExtraartists->get($id, contain: []);
        ...
        $albums = $this->AlbumsExtraartists->Albums->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $albumsExtraartist->name;
        $this->set(compact('albumsExtraartist', 'albums', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $albumsExtraartist = $this->AlbumsExtraartists->get($id);
        if ($this->AlbumsExtraartists->delete($albumsExtraartist)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
            //$this->Flash->success(__('The albums extraartist has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The albums extraartist could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
